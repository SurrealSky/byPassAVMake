...
STs32       extraspace;           // Extra space between arguments
STs32       putdefseg;            // Display default segments in listing
STs32       showmemsize;          // Always show memory size
STs32       shownear;             // Show NEAR modifiers
STs32       shortstringcmds;      // Use short form of string commands
STs32       sizesens;             // How to decode size-sensitive mnemonics
STs32       symbolic;             // Show symbolic addresses in disasm
STs32       farcalls;             // Accept far calls, returns & addresses
STs32       decodevxd;            // Decode VxD calls (Win95/98)
STs32       privileged;           // Accept privileged commands
STs32       iocommand;            // Accept I/O commands
STs32       badshift;             // Accept shift out of range 1..31
STs32       extraprefix;          // Accept superfluous prefixes
STs32       lockedbus;            // Accept LOCK prefixes
STs32       stackalign;           // Accept unaligned stack operations
STs32       iswindowsnt;          // When checking for dangers, assume NT
////////////////////////////////////////////////////////////////////////////////
...
static STs32       segprefix;            // Segment override prefix or SEG_UNDEF
static STs32       hasrm;                // Command has ModR/M byte
static STs32       hassib;               // Command has SIB byte
static STs32       dispsize;             // Size of displacement (if any)
static STs32       immsize;              // Size of immediate data (if any)
static STs32       softerror;            // Noncritical disassembler error
static STs32       ndump;                // Current length of command dump
static STs32       nresult;              // Current length of disassembly
static STs32       addcomment;           // Comment value of operand
...
ulong Disasm(char *src,ulong srcsize,ulong srcip,t_disasm *disasm,int disasmmode) 
{
  ...
  ndump=0; nresult=0;
  cmd=(unsigned char*)src; size=srcsize; pfixup=NULL;
  ...
  {                               // Command recognized, decode it
    ...
    if (pfixup!=NULL && da->fixupsize>0)
      da->fixupoffset=(int)(pfixup-(unsigned char*)src);
