...
struct CodeNode
{
	t_disasm	disasm;
	STbool		IsJmcType;//跳转
	STbool		IsJmcFromType;//从其他地方跳转过来的代码
	STbool		IsJmcBeSideType;//跳到外部的指令
	STbool		IsJmcUndefineType;//跳到代码内不明确的地址
	STbool		IsJmcDynamicType;//动态跳转类型	
	//STbool		IsJmcNextType;//JMC和CALL之后的代码

	STbool		IsCallType;//调用内部函数
	STbool		IsCallFromType;//从其他地方跳转过来的代码
	STbool		IsCallBeSideType;//跳到外部的指令
	STbool		IsCallUndefineType;//跳到代码内不明确的地址
	STbool		IsCallDynamicType;//动态跳转类型	
	//BOOL		IsCallNextType;//JMC和CALL之后的代码
	CodeNode()
	{
	...
	bool operator==(const ulong& immconst)const
	{
		return this->disasm.ip==immconst;
	}
	bool operator>(const CodeNode& node)const
	{
		return this->disasm.ip>node.disasm.ip;
